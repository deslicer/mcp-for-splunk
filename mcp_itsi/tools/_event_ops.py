"""Reusable CRUD operations against the ITSI Event Management Interface."""
...
async def list_objects(
    ...
    *,
    ...
    async with ctx.client() as client:
        result = await client.get_json(event_collection(object_type), params=params)
    return result if isinstance(result, list) else []


async def get_object(ctx: ITSICallContext, object_type: str, key: str) -> dict[str, Any] | None:
    async with ctx.client() as client:
        result = await client.get_json(event_item(object_type, key))
...
async def create_object(
    ...
    async with ctx.client() as client:
        result = await client.post_json(event_collection(object_type), body=payload)
...
async def update_object(
    ...
    *,
    ...
    async with ctx.client() as client:
        result = await client.post_json(event_item(object_type, key), body=payload, params=params)
    return result if isinstance(result, dict) else {"raw": result}


async def delete_object(ctx: ITSICallContext, object_type: str, key: str) -> dict[str, Any]:
    async with ctx.client() as client:
        return await client.delete(event_item(object_type, key))
