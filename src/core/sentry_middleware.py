...
class SentryMCPMiddleware(Middleware):
        ...
        try:
            ...
            # Then try context.message (FastMCP middleware context structure)
                ...
                if isinstance(message, dict):
                    ...
                    logger.debug(f"Found params in context.message: {type(params).__name__}, keys: {list(params.keys()) if isinstance(params, dict) else 'N/A'}")
            ...
            logger.debug(f"Params extracted: {type(params).__name__} with keys: {list(params.keys()) if isinstance(params, dict) else 'N/A'}")
            ...
            try:
                ...
                span.set_data("request.full_params", json.dumps(sanitized_params, default=str)[:4000])
            except Exception:
                pass
    ...
    def _sanitize_result(self, result: dict, max_depth: int = 3) -> dict:
        ...
        if not isinstance(result, dict) or max_depth <= 0:
            return result if not isinstance(result, str) or len(result) <= 500 else f"[{len(result)} chars]"
        
        sensitive_keys = {"password", "token", "secret", "authorization", "api_key", "apikey", "dsn"}
        sanitized = {}
        ...
        for key, value in result.items():
            ...
            if any(s in key_lower for s in sensitive_keys):
                ...
                if len(value) > 10:
                    ...
                    sanitized[key] = [
                        self._sanitize_result(item, max_depth - 1) if isinstance(item, dict) 
                        else (item if not isinstance(item, str) or len(item) <= 200 else f"[{len(item)} chars]")
